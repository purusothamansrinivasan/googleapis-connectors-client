...
func Connectors_projects_locations_connections_actions_executeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ExecuteActionRequest
		...
		url := fmt.Sprintf("%s/v2/%s:execute%s", cfg.BaseURL, name, queryString)
		...
		var result models.ExecuteActionResponse
...
func CreateConnectors_projects_locations_connections_actions_executeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_name_execute",
		mcp.WithDescription("Executes an action with the name specified in the request. The input parameters for executing the action are passed through the body of the ExecuteAction request."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Required. Resource name of the Action. Format: projects/{project}/locations/{location}/connections/{connection}/actions/{action}")),
		mcp.WithObject("parameters", mcp.Description("Input parameter: Parameters for executing the action. The parameters can be key/value pairs or nested structs.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_actions_executeHandler(cfg),
	}
}
