...
func Connectors_projects_locations_connections_actions_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/%s/actions%s", cfg.BaseURL, parent, queryString)
...
func CreateConnectors_projects_locations_connections_actions_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_parent_actions",
		mcp.WithDescription("Gets the schema of all the actions supported by the connector."),
		...
		mcp.WithString("parent", mcp.Required(), mcp.Description("Required. Parent resource name of the Action. Format: projects/{project}/locations/{location}/connections/{connection}")),
		mcp.WithNumber("pageSize", mcp.Description("Number of Actions to return. Defaults to 25.")),
		mcp.WithString("pageToken", mcp.Description("Page token, return from a previous ListActions call, that can be used retrieve the next page of content. If unspecified, the request returns the first page of actions.")),
		mcp.WithString("view", mcp.Description("Specifies which fields of the Action are returned in the response.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_actions_listHandler(cfg),
	}
}
