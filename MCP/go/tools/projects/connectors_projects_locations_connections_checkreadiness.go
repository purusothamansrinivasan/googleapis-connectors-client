...
func Connectors_projects_locations_connections_checkreadinessHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/%s:checkReadiness%s", cfg.BaseURL, name, queryString)
		...
		var result models.CheckReadinessResponse
...
func CreateConnectors_projects_locations_connections_checkreadinessTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_name_checkReadiness",
		mcp.WithDescription("Reports readiness status of the connector. Similar logic to GetStatus but modified for kubernetes health check to understand."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_checkreadinessHandler(cfg),
	}
}
