...
func Connectors_projects_locations_connections_checkstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/%s:checkStatus%s", cfg.BaseURL, name, queryString)
		...
		var result models.CheckStatusResponse
...
func CreateConnectors_projects_locations_connections_checkstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_name_checkStatus",
		mcp.WithDescription("Reports the status of the connection. Note that when the connection is in a state that is not ACTIVE, the implementation of this RPC method must return a Status with the corresponding State instead of returning a gRPC status code that is not "OK", which indicates that ConnectionStatus itself, not the connection, failed."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_checkstatusHandler(cfg),
	}
}
