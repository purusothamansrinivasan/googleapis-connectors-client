...
func Connectors_projects_locations_connections_entitytypes_entities_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateConnectors_projects_locations_connections_entitytypes_entities_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_parent_entities",
		mcp.WithDescription("Creates a new entity row of the specified entity type in the external system. The field values for creating the row are contained in the body of the request. The response message contains a `Entity` message object returned as a response by the external system."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_entities_createHandler(cfg),
	}
}
