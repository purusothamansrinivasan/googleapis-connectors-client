...
func Connectors_projects_locations_connections_entitytypes_entities_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateConnectors_projects_locations_connections_entitytypes_entities_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v2_name",
		mcp.WithDescription("Deletes an existing entity row matching the entity type and entity id specified in the request."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Required. Resource name of the Entity Type. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}/entities/{id}")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_entities_deleteHandler(cfg),
	}
}
