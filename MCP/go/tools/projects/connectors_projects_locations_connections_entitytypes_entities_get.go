...
func Connectors_projects_locations_connections_entitytypes_entities_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateConnectors_projects_locations_connections_entitytypes_entities_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_name",
		mcp.WithDescription("Gets a single entity row matching the entity type and entity id specified in the request."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_entities_getHandler(cfg),
	}
}
