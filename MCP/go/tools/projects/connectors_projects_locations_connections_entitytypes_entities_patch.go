...
func Connectors_projects_locations_connections_entitytypes_entities_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateConnectors_projects_locations_connections_entitytypes_entities_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v2_name",
		mcp.WithDescription("Updates an existing entity row matching the entity type and entity id specified in the request. The fields in the entity row that need to be modified are contained in the body of the request. All unspecified fields are left unchanged. The response message contains a `Entity` message object returned as a response by the external system."),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Output only. Resource name of the Entity. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}/entities/{id}")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_entities_patchHandler(cfg),
	}
}
