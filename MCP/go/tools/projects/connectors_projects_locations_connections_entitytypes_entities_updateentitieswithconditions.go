...
func Connectors_projects_locations_connections_entitytypes_entities_updateentitieswithconditionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/%s/entities:updateEntitiesWithConditions%s", cfg.BaseURL, entityType, queryString)
		...
		var result models.UpdateEntitiesWithConditionsResponse
...
func CreateConnectors_projects_locations_connections_entitytypes_entities_updateentitieswithconditionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_entityType_entities_updateEntitiesWithConditions",
		mcp.WithDescription("Updates entities based on conditions specified in the request and not on entity id."),
		...
		mcp.WithString("entityType", mcp.Required(), mcp.Description("Required. Resource name of the Entity Type. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}")),
		mcp.WithString("conditions", mcp.Description("Required. Conditions to be used when updating entities. From a proto standpoint, There are no restrictions on what can be passed using this field. The connector documentation should have information about what format of filters/conditions are supported. Note: If this conditions field is left empty, an exception is thrown. We don't want to consider 'empty conditions' to be a match-all case. Connector developers can determine and document what a match-all case constraint would be.")),
		mcp.WithObject("fields", mcp.Description("Input parameter: Fields of the entity. The key is name of the field and the value contains the applicable `google.protobuf.Value` entry for this field.")),
		mcp.WithString("name", mcp.Description("Input parameter: Output only. Resource name of the Entity. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}/entities/{id}")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_entities_updateentitieswithconditionsHandler(cfg),
	}
}
