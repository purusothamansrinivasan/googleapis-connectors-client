...
func Connectors_projects_locations_connections_entitytypes_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/%s/entityTypes%s", cfg.BaseURL, parent, queryString)
		...
		var result models.ListEntityTypesResponse
...
func CreateConnectors_projects_locations_connections_entitytypes_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_parent_entityTypes",
		mcp.WithDescription("Lists metadata related to all entity types present in the external system."),
		...
		mcp.WithString("parent", mcp.Required(), mcp.Description("Required. Resource name of the Entity Type. Format: projects/{project}/locations/{location}/connections/{connection}")),
		mcp.WithNumber("pageSize", mcp.Description("Number of entity types to return. Defaults to 25.")),
		mcp.WithString("pageToken", mcp.Description("Page token, return from a previous ListEntityTypes call, that can be used retrieve the next page of content. If unspecified, the request returns the first page of entity types.")),
		mcp.WithString("view", mcp.Description("Specifies which fields of the Entity Type are returned in the response.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_entitytypes_listHandler(cfg),
	}
}
