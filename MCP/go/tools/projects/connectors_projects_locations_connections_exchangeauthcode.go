...
func Connectors_projects_locations_connections_exchangeauthcodeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ExchangeAuthCodeRequest
		...
		url := fmt.Sprintf("%s/v2/%s:exchangeAuthCode%s", cfg.BaseURL, name, queryString)
...
func CreateConnectors_projects_locations_connections_exchangeauthcodeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_name_exchangeAuthCode",
		mcp.WithDescription("ExchangeAuthCode exchanges the OAuth authorization code (and other necessary data) for an access token (and associated credentials)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_exchangeauthcodeHandler(cfg),
	}
}
