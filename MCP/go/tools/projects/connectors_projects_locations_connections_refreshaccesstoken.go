...
func Connectors_projects_locations_connections_refreshaccesstokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.RefreshAccessTokenRequest
		...
		url := fmt.Sprintf("%s/v2/%s:refreshAccessToken%s", cfg.BaseURL, name, queryString)
		...
		var result models.RefreshAccessTokenResponse
...
func CreateConnectors_projects_locations_connections_refreshaccesstokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_name_refreshAccessToken",
		mcp.WithDescription("RefreshAccessToken exchanges the OAuth refresh token (and other necessary data) for a new access token (and new associated credentials)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Connectors_projects_locations_connections_refreshaccesstokenHandler(cfg),
	}
}
