/**
 * MCP Server function for Reports readiness status of the connector. Similar logic to GetStatus but modified for kubernetes health check to understand.
 */
...
class Get_V2_Name_Check_ReadinessMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_V2_Name_Check_ReadinessHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_v2_name_check_readiness" + queryString;
    ...
    public static MCPServer.Tool createGet_V2_Name_Check_ReadinessTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_V2_Name_Check_ReadinessHandler(config));
    }
