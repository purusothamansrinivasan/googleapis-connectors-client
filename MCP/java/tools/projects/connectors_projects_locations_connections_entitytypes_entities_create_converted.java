/**
 * MCP Server function for Creates a new entity row of the specified entity type in the external system. The field values for creating the row are contained in the body of the request. The response message contains a `Entity` message object returned as a response by the external system.
 */
...
class Post_V2_Parent_EntitiesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_V2_Parent_EntitiesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_v2_parent_entities" + queryString;
    ...
    public static MCPServer.Tool createPost_V2_Parent_EntitiesTool(MCPServer.APIConfig config) {
        ...
        parentProperty.put("description", "Required. Resource name of the Entity Type. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}");
        ...
        return new MCPServer.Tool(definition, getPost_V2_Parent_EntitiesHandler(config));
    }
