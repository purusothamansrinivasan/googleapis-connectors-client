/**
 * MCP Server function for Updates an existing entity row matching the entity type and entity id specified in the request. The fields in the entity row that need to be modified are contained in the body of the request. All unspecified fields are left unchanged. The response message contains a `Entity` message object returned as a response by the external system.
 */
...
class Patch_V2_NameMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPatch_V2_NameHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/patch_v2_name" + queryString;
    ...
    public static MCPServer.Tool createPatch_V2_NameTool(MCPServer.APIConfig config) {
        ...
        nameProperty.put("description", "Output only. Resource name of the Entity. Format: projects/{project}/locations/{location}/connections/{connection}/entityTypes/{type}/entities/{id}");
        ...
        return new MCPServer.Tool(definition, getPatch_V2_NameHandler(config));
    }
