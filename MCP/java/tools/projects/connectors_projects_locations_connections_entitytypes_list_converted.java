/**
 * MCP Server function for Lists metadata related to all entity types present in the external system.
 */
...
class Get_V2_Parent_Entity_TypesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_V2_Parent_Entity_TypesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("view")) {
            queryParams.add("view=" + args.get("view"));
        }
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_v2_parent_entity_types" + queryString;
    ...
    public static MCPServer.Tool createGet_V2_Parent_Entity_TypesTool(MCPServer.APIConfig config) {
        ...
        parentProperty.put("description", "Required. Resource name of the Entity Type. Format: projects/{project}/locations/{location}/connections/{connection}");
        ...
        pageTokenProperty.put("description", "Page token, return from a previous ListEntityTypes call, that can be used retrieve the next page of content. If unspecified, the request returns the first page of entity types.");
        properties.put("pageToken", pageTokenProperty);
        Map<String, Object> viewProperty = new HashMap<>();
        viewProperty.put("type", "string");
        viewProperty.put("required", false);
        viewProperty.put("description", "Specifies which fields of the Entity Type are returned in the response.");
        properties.put("view", viewProperty);
        Map<String, Object> pageSizeProperty = new HashMap<>();
        pageSizeProperty.put("type", "string");
        pageSizeProperty.put("required", false);
        pageSizeProperty.put("description", "Number of entity types to return. Defaults to 25.");
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "get_v2_parent_entity_types",
            "Lists metadata related to all entity types present in the external system.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getGet_V2_Parent_Entity_TypesHandler(config));
    }
